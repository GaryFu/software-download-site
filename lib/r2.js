...
export async function registerPackage(metadata) {
  ...
  const normalizedKey = packageReleaseKey(normalized);
  const packages = normalizeCatalogPackages([
    normalized,
    ...current.packages.filter(
      (item) => item.objectKey !== normalized.objectKey && packageReleaseKey(item) !== normalizedKey,
    ),
  ]);
  const catalog = {
...
function packageIdentity(item) {
  return normalizedIdentity(item.appName) || normalizedIdentity(item.packageName);
}
...
function packageReleaseKey(item) {
  ...
  const version = normalizedIdentity(item.version);
  if (version) {
    return `release:${identity}|${version}`;
  }
  const sha256 = normalizedIdentity(item.sha256);
  if (sha256) {

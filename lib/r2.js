...
export async function registerPackage(metadata) {
  ...
  const normalizedKey = packageDedupKey(normalized);
  const packages = normalizeCatalogPackages([
    normalized,
    ...current.packages.filter((item) => item.objectKey !== normalized.objectKey && packageDedupKey(item) !== normalizedKey),
  ]);
  const catalog = {
...
function packageDedupKey(item) {
  if (item.sha256) {
    return `sha256:${String(item.sha256).toLowerCase()}`;
  }
  return [item.appName, item.version, item.fileName]
...
function normalizeCatalogPackages(packages) {
  ...
  packages.map(normalizePackage).forEach((item) => {
    ...
    const key = packageDedupKey(item);

...
- Admin-only upload flow protected by `UPLOAD_TOKEN`.
...
Download does not require a user key. If `CLOUDFLARE_R2_PUBLIC_BASE_URL` is not a public R2 domain, `/api/download` redirects to a short-lived signed R2 URL instead.
